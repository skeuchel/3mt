...
Section Test_Section.
  ...
  Definition D := AType :+: BType :+: LType :+: UnitType :+: RefType.
  Definition E (A : Set) := Arith :+: Bool :+: Lambda D A :+: RefE :+: ExceptE D.
  ...
  Proof.
    ...
    apply eqv_soundness_X'_Bool_eqv with (WFV := WFV) (Fail_MT := Fail_MT)
      (monad := monad0) (TypContextCE := SigGamma_GammaTypContextCE _)
      (Sub_BType_D := _) (Sub_StuckValue_V := _) (Sub_BoolValue_V := _)
    ...
    apply eqv_soundness_X'_RefE_eqv with  (WFV := WFV) (TypContext_S := _)
      (TypContextCE := SigGamma_GammaTypContextCE _)
      (TypContext_WFE := SigGamma_WF_EnvC _ _ WFV) (Fail_MT := Fail_MT) (StateM_ME := State_ME)
      (Sub_RefType_D := _) (Sub_StuckValue_V := _) (Sub_LocValue_V := _)
