...
Section Test_Section.
  ...
  Definition E (A : Set) := Bool :+: Lambda D A :+: ExceptE D.
  ...
  Definition V := StuckValue :+: BoolValue :+: (ClosValue E).
  ...
  Definition EQV_E (A B : Set) := (Bool_eqv E A B)
    ::+:: (Lambda_eqv D E A B) ::+:: (Exception_eqv D E A B).
  ...
  Proof.
    ...
    apply eqv_soundness_X'_Bool_eqv with (WFV := WFV) (TypContextCE := GammaTypContextCE _)
    ...
    apply iPAlgebra_Plus.
    apply eqv_soundness_X'_Lambda_eqv with (WFV := WFV) (TypContextCE := GammaTypContextCE _)
  ...
  Context {fail_neq_throw : forall A,
    fail (M := ME) <> throw (A := A) tt}.
  ...
  Eval compute in ("Type Soundness for Bool + Lambda + Exception Proven!").
