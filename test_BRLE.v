...
Require Import Lambda_Exception_State.

Section Test_Section.

  Definition D := BType :+: LType :+: UnitType :+: RefType.
  Definition E (A : Set) := Bool :+: Lambda D A :+: RefE :+: ExceptE D.
  ...
  Definition EQV_E (A B : Set) := (Bool_eqv E A B)
    ::+:: (Lambda_eqv D E A B) ::+:: (RefE_eqv E A B)
    ::+:: (Exception_eqv D E A B).

  Definition WFV := (WFValue_VB D V (list (Names.DType D) * list (Names.DType D))) ::+::
    (WFValue_Clos D E MT V EQV_E _)
  ...
  Proof.
    ...
    apply eqv_soundness_X'_Bool_eqv with (WFV := WFV) (TypContextCE := SigGamma_GammaTypContextCE _)
      (Fail_MT := Fail_MT) (monad := monad0) (Sub_BType_D := _)
      (Sub_StuckValue_V := _) (Sub_BoolValue_V := _)
      (eq_DType_DT := _); intros; eauto 550 with typeclass_instances.
    apply eqv_soundness_X'_Lambda_eqv with (WFV := WFV) (TypContextCE := SigGamma_GammaTypContextCE _)
    ...
    apply eqv_soundness_X'_RefE_eqv with (WFV := WFV) (TypContext_S := _)
      (TypContextCE := SigGamma_GammaTypContextCE _)
      (TypContext_WFE := SigGamma_WF_EnvC _ _ WFV)
      (StateM_ME := State_ME) (Fail_MT := Fail_MT) (Sub_RefType_D := _)
    ...
    apply eqv_soundness_X'_Except_eqv with (TypContextCE := SigGamma_GammaTypContextCE _)
  ...
  Eval compute in ("Type Soundness for Arith + Bool + Ref + Lambda + Exception Proven!").
