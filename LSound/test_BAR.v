...
Section Test_Section.
  ...
  Definition D := AType :+: BType :+: UnitType :+: RefType.

  Definition E := Arith :+: Bool :+: RefE.

  Definition V := StuckValue :+: NatValue :+: BoolValue :+: LocValue :+: UnitValue.
  ...
  Definition WFV :=
    (WFValue_VI V D (list (Names.DType D))) ::+::
    (WFValue_VB D V _) ::+:: (WFValue_Unit D V _) ::+::
    (WFValue_Loc D V _).
  ...
  Eval compute in ("Soundness for 'Arith :+: Bool :+: References' Proven!").
