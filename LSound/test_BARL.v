...
Section Test_Section.
  ...
  Definition D := BType :+: AType :+: LType :+: UnitType :+: RefType.
  Definition E (A : Set) := Bool :+: Arith :+: Lambda D A :+: RefE.
  ...
  Definition V := StuckValue :+: BoolValue :+: NatValue :+: (ClosValue E) :+: LocValue :+: UnitValue.
  ...
  Definition EQV_E (A B : Set) := (Bool_eqv E A B) ::+:: (Arith_eqv E A B)
    ::+:: (Lambda_eqv D E A B) ::+:: (RefE_eqv E A B).

  Definition WFV := (WFValue_VB D V (list (Names.DType D) * list (Names.DType D)))
    ::+:: (WFValue_VI V D _) ::+:: (WFValue_Clos D E MT V EQV_E _)
  ...
  Proof.
    ...
    apply eqv_soundness_X'_Bool_eqv with (WFV := WFV) (TypContextCE :=  _)
  ...
  Eval compute in ("Type Soundness for Bool + Arith + Lambda + Ref Proven!").
