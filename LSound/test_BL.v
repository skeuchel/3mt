...
Section Test_Section.
  ...
  Definition EQV_E (A B : Set) := (Bool_eqv E A B) ::+:: (Lambda_eqv D E A B).

  Definition WFV := (WFValue_VB D V (list (DType D))) ::+:: (WFValue_Clos D E MT V EQV_E (list (Names.DType D))).
  ...
  Proof.
    ...
    apply eqv_soundness_X'_Bool_eqv with (WFV := WFV) (TypContextCE := GammaTypContextCE _)
      ...
      (Sub_StuckValue_V := _) (Sub_BoolValue_V := _) (eq_DType_DT := _);
  ...
  Eval compute in ("Type Soundness for Bool + Lambda Proven!").
