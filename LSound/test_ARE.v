...
Section Test_Section.
  ...
  Definition D := AType :+: RefType :+: UnitType.

  Definition E := Arith :+: RefE :+: (ExceptE D).

  Definition V := StuckValue :+: NatValue :+: UnitValue :+: LocValue.
  ...
  Proof.
    ...
    apply FAlgebra_Plus; eauto with typeclass_instances.
    apply MAlgebra_typeof_RefE with (Fun_MT := functor) (Mon_MT := monad);
  ...
  Proof.
    ...
    apply FAlgebra_Plus; eauto with typeclass_instances.
    apply FAlgebra_Plus; eauto with typeclass_instances.
    apply MAlgebra_evalM_RefE with (Fun_M := functor0) (Mon_M := monad0);
      eauto with typeclass_instances.
  Defined.
  ...
  Eval compute in ("Soundness for 'Arith :+: Ref :+: Exceptions ' Proven!").
