...
Section Test_Section.
  ...
  Definition D := LType :+: UnitType :+: RefType.
  Definition E (A : Set) := Lambda D A :+: RefE :+: ExceptE D.
  ...
  Definition V := StuckValue :+: (ClosValue E) :+: LocValue :+: UnitValue.
  ...
  Definition EQV_E (A B : Set) := (Lambda_eqv D E A B) ::+:: (RefE_eqv E A B)
    ::+:: (Exception_eqv D E A B).

  Definition WFV :=(WFValue_Clos D E MT V EQV_E _) ::+::
    (WFValue_Unit D V (list (Names.DType D) * list (Names.DType D))) ::+:: (WFValue_Loc D V _).
  ...
  Proof.
    ...
    apply eqv_soundness_X'_Lambda_eqv with (WFV := WFV) (TypContextCE := SigGamma_GammaTypContextCE _)
      (Fail_MT := Fail_MT) (Environment_ME := Mon_ME) (Sub_LType_D := _)
  ...
  Eval compute in ("Type Soundness for Ref + Lambda + Exception Proven!").
