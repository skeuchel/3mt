...
Section Ref.
  ...
  Context {ExceptionM : Exception ME nat}.
  ...
  (* ============================================== *)
    ...
    Variable Sub_WFV_Unit_WFV : Sub_iFunctor WFValue_Unit WFV.
    ...
    Context {TypContext_S : SigTypContextC TypContext}.
    Context {TypContext_SCE : SigConsExtensionC TypContext _ _}.
    ...
    Variable Sub_WFV_Loc_WFV : Sub_iFunctor WFValue_Loc WFV.
  ...
  (* ============================================== *)
    ...
    Context {TypContext_SWFE : Sig_WF_EnvC TypContext WFV _ _}.
    ...
    Proof.
      ...
      intros.
      apply (inject_i (subGF := Sub_WFVM_State_WFVM)).
      econstructor 2 with (Sigma' := SigInsert T Sigma'); simpl.
      apply WF_EnvInsert; auto.
      apply (WFV_Weaken _ H0); simpl; auto.
      apply ConsExtension_SigInsert.
      apply ConsExtension_SigInsert.
    ...
    Proof.
      ...
      generalize (WF_invertLoc' TypContext WFV _ _ H0 _ H1) as WF_v_T; intros;
      ...
      destruct (WF_EnvLookup _ _ _ _ WF_env_Sigma H6) as [v' [lookup_v' WF_v'_T']];
      ...
      destruct r; apply project_inject in H2; generalize (WF_invertLoc _ _ _ _ H0 _ (proj2_sig s));
    ...
    Proof.
      ...
      generalize (WF_invertLoc' _ _ _ _ H0 _ H1) as WF_v_T;
      ...
      apply (ConsExtension_SigLookup _ Sigma'0) in H5; auto.
      apply (WF_EnvReplace _ _ _ T'); auto.
  ...
  Section State_Sound_Sec.
    ...
    Global Instance DType_Env_S : SigTypContextC (list DType) :=
      {| SigLookup := lookup;
        SigInsert := insert _|}.

    Global Instance DType_Env_WFE : WF_EnvC (list DType) :=
      {| WF_Env := fun env Sigma =>
        P2_Env (WFValueC _ _ _ WFV' Sigma) env Sigma |}.

    Global Instance DType_Env_SCE : SigConsExtensionC (list DType) _ _.
    Proof.
      constructor.
    ...
    Global Instance DType_Env_SWFE : Sig_WF_EnvC (list DType) WFV' _ _.
    Proof.
    ...
    Global Instance State_Sound_WFVM_State :
      iPAlgebra State_Sound_Name State_Sound_P (WFValueM_State (list DType)).
    Proof.
      econstructor.
      unfold iAlgebra; intros; apply ind_alg_WFVM_State with (TypContextCE := DType_Env_CE)
